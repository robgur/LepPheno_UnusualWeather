...
inla_mdf <- mdf_phylo %>% 
  dplyr::select(dur, annualTemp, precSeas, annualPrec, dstdoy, voltinism, overwinteringStage,
...
dvt <- ggplot(rdf_total, mapping = aes(x = V1, y = mean)) +
  geom_ribbon(mapping = aes(ymin = `0.025quant`, ymax = `0.975quant`,
                            fill = V2), 
              alpha = 0.15) +
  geom_line(mapping = aes(color = V2), size = 1.05) +
  labs(x = "Temperature", y = "Duration", fill = "Voltinism", color = "Voltinism") + 
...
save(dvt, file = "singleInteractionFigures/dur_voltinism_temp.Rdata")
