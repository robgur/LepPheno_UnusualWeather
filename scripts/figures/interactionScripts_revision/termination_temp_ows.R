...
pglmm3 <- pglmm(formula = q95 ~ annualTemp + annualPrec + 
                  dstdoy + 
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:voltinism +
                  annualTemp:overwinteringStage +
...
pglmm4 <- pglmm(formula = q95 ~ annualTemp + tempSeas + precSeas +
                  dstdoy + 
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:voltinism +
                  annualTemp:overwinteringStage +
...
pglmm5 <- pglmm(formula = q95 ~ annualTemp + tempSeas + precSeas +
                  dstdoy + 
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:voltinism +
                  annualTemp:overwinteringStage +
...
tot <- ggplot(rdf_total, mapping = aes(x = V1, y = mean)) +
  geom_ribbon(mapping = aes(ymin = `0.025quant`, ymax = `0.975quant`,
                            fill = V2), 
              alpha = 0.15) +
  geom_line(mapping = aes(color = V2), size = 1.05) +
  labs(x = "Temperature", y = "Termination", fill = "Diapause stage", color = "Diapause stage") + 
  scale_fill_viridis_d() +
  scale_color_viridis_d() +
  ggtitle(label = "Termination") + 
  theme_classic() +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5)) 

tot

save(tot, file = "singleInteractionFigures/termination_overwintering_temp.Rdata")
