...
inla_mdf <- mdf_phylo %>% 
  dplyr::select(q5, annualTemp, precSeas, annualPrec, dstdoy, voltinism, overwinteringStage,
...
pglmm1 <- pglmm(formula = q5 ~ annualTemp + annualPrec + precSeas +
                  dstdoy +
                  overwinteringStage + 
...
pglmm2 <- pglmm(formula = q5 ~ annualTemp + annualPrec + precSeas +
                  dstdoy +
                  overwinteringStage + 
                  annualTemp:overwinteringStage +
                  year:overwinteringStage +
...
pglmm4 <- pglmm(formula = q5 ~ annualTemp + annualPrec + precSeas +
                  dstdoy +
                  overwinteringStage + 
                  annualTemp:overwinteringStage +
                  year:overwinteringStage +
...
pglmm5 <- pglmm(formula = q5 ~ annualTemp + annualPrec + precSeas +
                  dstdoy +
                  overwinteringStage + 
                  annualTemp:overwinteringStage +
                  year:overwinteringStage +
...
oot <- ggplot(rdf_total, mapping = aes(x = V1, y = mean)) +
  geom_ribbon(mapping = aes(ymin = `0.025quant`, ymax = `0.975quant`,
                            fill = V2), 
              alpha = 0.15) +
  geom_line(mapping = aes(color = V2), size = 1.05) +
  labs(x = "Temperature", y = "Onset", fill = "Diapause stage", color = "Diapause stage") + 
...
save(oot, file = "singleInteractionFigures/onset_overwintering_temp.Rdata")
