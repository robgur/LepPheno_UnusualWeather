...
pglmm4 <- pglmm(formula = q50 ~ annualTemp + tempSeas + annualPrec +
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:overwinteringStage +
                  annualTemp:Seas +
                  tempSeas:year +
...
pglmm5 <- pglmm(formula = q50 ~ annualTemp + tempSeas + annualPrec +
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:overwinteringStage +
                  annualTemp:Seas +
                  tempSeas:year +
...
mot <- ggplot(rdf_total, mapping = aes(x = V1, y = mean)) +
  geom_ribbon(mapping = aes(ymin = `0.025quant`, ymax = `0.975quant`,
                            fill = V2), 
              alpha = 0.15) +
  geom_line(mapping = aes(color = V2), size = 1.05) +
  labs(x = "Temperature", y = "Midpoint", fill = "Diapause stage", color = "Diapause stage") + 
  scale_fill_viridis_d() +
  scale_color_viridis_d() +
  ggtitle(label = "Midpoint") + 
  theme_classic() +
...
save(mot, file = "singleInteractionFigures/midpoint_overwintering_temp.Rdata")
