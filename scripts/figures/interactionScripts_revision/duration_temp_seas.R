...
inla_mdf <- mdf_phylo %>% 
  dplyr::select(dur, annualTemp, annualPrec, precSeas, dstdoy, voltinism, overwinteringStage,
...
pglmm2 <- pglmm(formula = dur ~ annualTemp + annualPrec + precSeas +
                  dstdoy +
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:voltinism +
                  annualTemp:Seas + 
...
pglmm3 <- pglmm(formula = dur ~ annualTemp + annualPrec + precSeas +
                  dstdoy +
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:voltinism +
                  annualTemp:Seas + 
...
dst <- ggplot(rdf_total, mapping = aes(x = V1, y = mean)) +
  geom_ribbon(mapping = aes(ymin = `0.025quant`, ymax = `0.975quant`,
                            fill = V2), 
              alpha = 0.15) +
  geom_line(mapping = aes(color = V2), size = 1.05) +
  labs(x = "Temperature", y = "Duration", fill = "Seasonality", color = "Seasonality") + 
...
save(dst, file = "singleInteractionFigures/dur_Seasonality_temp.Rdata")
