...
ooy <- ggplot(rdf_total, mapping = aes(x = V1, y = mean)) +
  geom_ribbon(mapping = aes(ymin = `0.025quant`, ymax = `0.975quant`,
                            fill = V2), 
              alpha = 0.15) +
  geom_line(mapping = aes(color = V2), size = 1.05) +
  labs(x = "Year", y = "Onset", fill = "Diapause stage", color = "Diapause stage") + 
...
save(ooy, file = "singleInteractionFigures/onset_overwintering_year.Rdata")
