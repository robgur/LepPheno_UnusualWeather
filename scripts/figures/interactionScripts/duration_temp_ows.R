...
inla_mdf <- mdf_phylo %>% 
  dplyr::select(dur, annualTemp, annualPrec, dstdoy, voltinism, overwinteringStage,
...
pglmm2 <- pglmm(formula = dur ~ annualTemp + annualPrec +
                  dstdoy +
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:voltinism +
                  annualTemp:overwinteringStage +
...
pglmm3 <- pglmm(formula = dur ~ annualTemp + annualPrec +
                  dstdoy +
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:voltinism +
                  annualTemp:overwinteringStage +
...
pglmm4 <- pglmm(formula = dur ~ annualTemp + annualPrec +
                  dstdoy +
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:voltinism +
                  annualTemp:overwinteringStage +
...
pglmm5 <- pglmm(formula = dur ~ annualTemp + annualPrec +
                  dstdoy +
                  voltinism + overwinteringStage + diurnality + Seas +
                  annualTemp:voltinism +
                  annualTemp:overwinteringStage +
...
dot <- ggplot(rdf_total, mapping = aes(x = V1, y = mean)) +
  geom_ribbon(mapping = aes(ymin = `0.025quant`, ymax = `0.975quant`,
                            fill = V2), 
              alpha = 0.15) +
  geom_line(mapping = aes(color = V2), size = 1.05) +
  labs(x = "Temperature", y = "Duration", fill = "Diapause stage", color = "Diapause stage") + 
...
save(dot, file = "singleInteractionFigures/dur_overwintering_temp.Rdata")
