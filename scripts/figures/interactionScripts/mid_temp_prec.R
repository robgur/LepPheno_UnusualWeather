...
mpt <- ggplot(rdf_total, mapping = aes(x = V1, y = mean)) +
  geom_ribbon(mapping = aes(ymin = `0.025quant`, ymax = `0.975quant`,
                            fill = V2), 
              alpha = 0.15) +
  geom_line(mapping = aes(color = V2), size = 1.05) +
  labs(x = "Temperature", y = "Midpoint", fill = "Precipitation", color = "Precipitation") + 
...
save(mpt, file = "singleInteractionFigures/mid_prec_temp.Rdata")
